...
def get_source_target_data(nba_data):
    columns_to_keep = [
    "Team", "Age", "GamesPlayed", "Wins", "Losses", "MinutesPlayed",
    "PointsScored", "FG%", "3P%", "FT%", "TotalRebounds", "Assists", 
    "Turnovers", "Steals", "Blocks", "PersonalFouls", "Year", "NetScoreOfPlayer"
    ]   
...
def create_folders(experiment_parameters):
    main_folder_path = f'NBA_Data_Analysis/{experiment_parameters['instance_1'].replace(' ','')}_vs_{experiment_parameters['instance_2'].replace(' ','')}_{experiment_parameters['independent_var1']}_{experiment_parameters['independent_var2']}'
...
def write_file_paths(path_source_target, path_distances, path_baseline, path_regression, notebook_folder_path):
    with open('NBA_Data_Analysis/File_Paths/path_source_target.txt', 'w') as f:
        f.write(path_source_target)
    with open('NBA_Data_Analysis/File_Paths/path_distances.txt', 'w') as f:
        f.write(path_distances)
    with open('NBA_Data_Analysis/File_Paths/path_baseline.txt', 'w') as f:
        f.write(path_baseline)
    with open('NBA_Data_Analysis/File_Paths/path_regression.txt', 'w') as f:
        f.write(path_regression)
    with open('NBA_Data_Analysis/File_Paths/notebook_folder_path.txt', 'w') as f:
